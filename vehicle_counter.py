...
# get live video from youtube (if ypu want to use YT live video)
...
with open("datasets/sequential_data/traffic_volume_detections_with_accuracy.csv", "w", newline="") as csvfile:
    headernames = ["ID", "VEHICLE", "CONFIDENCE"]
    writer = csv.DictWriter(csvfile, fieldnames=headernames)
    writer.writeheader()

with open("datasets/sequential_data/traffic_volume_data(5_min_interval).csv", "w", newline="") as csvfile:
...
# below is local video file path (uncomment it and comment the one above)
# cap = cv2.VideoCapture("PASTE_FILE_PATH_HERE")
